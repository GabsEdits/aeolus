...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const baseUrl: string = "https://gabs.eu.org/aeolus/assets/";

  const svgOpen: HTMLImageElement | null = document.getElementById(
    "svg-open"
  ) as HTMLImageElement;
  if (svgOpen) {
    svgOpen.src = prefersDarkMode
      ? baseUrl + "open-dark.svg"
      : baseUrl + "open.svg";
  }

  const svgSecure: HTMLImageElement | null = document.getElementById(
    "svg-secure"
  ) as HTMLImageElement;
  if (svgSecure) {
    svgSecure.src = prefersDarkMode
      ? baseUrl + "secure-dark.svg"
      : baseUrl + "secure.svg";
  }

  const svgManageable: HTMLImageElement | null = document.getElementById(
    "svg-manageable"
  ) as HTMLImageElement;
  if (svgManageable) {
    svgManageable.src = prefersDarkMode
      ? baseUrl + "manageable-dark.svg"
      : baseUrl + "manageable.svg";
  }
});
